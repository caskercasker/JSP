...
/*	
	...
	*/
	public FoodHouseBean foodDetailData(int no){
		...
		try {
			getConnection();
			String sql="SELECT title,score,address,tel,type,price,image,good,soso,bad,no "
					+"FROM foodhouse "
					+"WHERE no=?";
			ps=conn.prepareStatement(sql);
			ps.setInt(1, no);
...
//			rs.close();
			//이미지가 혹은 값이 비어 잇는경우가 있음..
			//rs close() 했는데 결과값을 받아올 경우 결과 집합을 모두 소모했음.
		} catch (Exception e) {
			// TODO: handle exception
			e.printStackTrace();
		}finally{
